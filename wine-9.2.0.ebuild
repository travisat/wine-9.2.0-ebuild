...
if [[ ${PV} == "9999" ]] ; then
	...
	inherit autotools
	MY_P="${PN}-${PV/_/-}"
	SRC_URI="http://media.codeweavers.com/pub/crossover/source/crossover-games-sources-${PV}.tar.gz"
...
src_unpack() {
	...
	if [[ ${PV} == "9999" ]] ; then
		...
		unpack crossover-games-sources-${PV}.tar.gz
		rm -r freetype
		rm -r cxgui
		rm -r loki
		rm -r samba
...
src_configure() {
	...
	do_configure 32 --disable-wine64
}
...
pkg_postinst() {
	...
	cp /usr/bin/wineloader /usr/bin/wine
}
